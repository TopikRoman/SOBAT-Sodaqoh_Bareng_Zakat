...
const useLogin = (navigation) => {
  ...
  const handleLogin = useCallback(async () => {
    ...
    try {
      ...
      await checkIfAccountIsApproved(email, navigation);
    } catch (error) {
...
const checkIfAccountIsApproved = async (email, navigation) => {
  const db = getFirestore();
  const amilRef = collection(db, "dataAmilApproved");
  const q = query(amilRef, where("email", "==", email));
  try {
    ...
    if (!querySnapshot.empty) {
      ...
      navigation.navigate("MainMenuMuzakki", { userEmail: email });
    }
  } catch (error) {
